...
function PANEL:Init()
	...
	close.SetText = function(this, text, noTranslation)
		DButton.SetText(this, noTranslation and text or L(text))
	end
	close:SetText("G")
	close:SetFont("ixIconsMenuButton")
	...
	characters.SetText = function(this, text, noTranslation)
		DButton.SetText(this, noTranslation and text or L(text))
	end
	characters:SetText("s")
	characters:SetFont("ixIconsMenuButton")
	characters:SetContentAlignment(5)
	characters:Dock(LEFT)
	characters:SetBackgroundColor(ix.config.Get("color"))
	characters.Paint = function(this, width, height)
		local alpha = this.currentBackgroundAlpha
		local active = Lerp(alpha/255, 0, 20)
	...
	tabs["settings"] = {
		PopulateTabButton = function(info, button)
			...
			button.Paint = function(panel, width, height)
				local alpha = panel.selected and 255 or panel.currentBackgroundAlpha

				surface.SetTexture(surface.GetTextureID("vgui/gradient-l"))
				surface.SetDrawColor(ColorAlpha(panel.backgroundColor, alpha))
				surface.DrawTexturedRectRotated(width/2-1, height/2, width+1, height, 180)
